                    ...
                    <form method="POST" action="{{ route('kelas.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="nama_dosen" id="nama_dosen"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                value="{{ old('nama_dosen', Auth::user()->name) }}" required> {{-- Default isi dengan nama user yang login --}}
                            @error('nama_dosen')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="kode_kelas" id="kode_kelas"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                value="{{ old('kode_kelas', $randomCode ?? '') }}" required>
                            <p class="mt-1 text-xs text-gray-500">
                                Kode kelas harus unik (contoh: IF301, PWEB101), atau
