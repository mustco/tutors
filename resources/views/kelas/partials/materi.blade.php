...
<div class="bg-blue-50 border border-blue-200 px-3 py-2 rounded-lg text-gray-700 flex justify-between items-center mb-6">
    <p class="font-semibold">Klik <span class="text-blue-600 font-bold">"Buat Materi"</span> untuk membuat pengumuman baru.</p>
...
<div class="bg-white border border-blue-300 rounded-lg p-4 shadow-sm">
    ...
    <div class="flex flex-col justify-center items-center">
        ...
        <div class="text-center text-gray-500 pt-4 text-xl font-bold">Tidak ada Materi apapun</div>
        @if (Auth::check() && Auth::user()->isAdmin())
        <div class="text-center text-gray-500 pb-4">Kamu belum membuat materi di kelas ini.</div>
        @else
        <div class="text-center text-gray-500 pb-4">Horee, pengajar belum membuat materi di kelas ini.</div>
        @endif
    </div>
    @else
        @foreach($kelas->pengumuman as $pengumuman)
            <div class="border-b border-blue-300 pb-3 mb-3 last:border-b-0 last:pb-0 last:mb-0">
                ...
                @endif
                <p class="text-sm text-gray-500 mt-1">Materi baru oleh {{ $kelas->nama_dosen }} pada {{ $pengumuman->created_at->format('d M Y, H:i') }}</p>
            </div>
        @endforeach
    @endif
</div>